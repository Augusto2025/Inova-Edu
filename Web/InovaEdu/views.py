...
def perfil(request):
    ...
    # Pegar projetos do usuário
    if usuario.tipo == 'Professor':
        # Para professor, pegamos todas as turmas e seus projetos
        # Ou você pode definir uma lógica específica aqui
        projetos = Projeto.objects.all().select_related('id_turma__id_curso')[:20]
    else:
        # Aluno vê projetos das turmas que ele participa
        turmas_do_aluno = UsuarioDaTurma.objects.filter(id_usuario=usuario).values_list('id_turma', flat=True)
        projetos = Projeto.objects.filter(id_turma__in=turmas_do_aluno).select_related('id_turma__id_curso')
...
def atualizar_perfil_ajax(request):
    print("="*50)
    print("FUNÇÃO: atualizar_perfil_ajax")
    print("Método:", request.method)
    print("Session:", request.session.get('usuario_email'))
...
def upload_foto_ajax(request):
    ...
    if request.method == 'POST' and request.FILES.get('foto'):
        ...
        try:
            ...
            print("Foto recebida:", foto.name, foto.size, foto.content_type)
            ...
            result = cloudinary.uploader.upload(
                ...
                public_id=f"usuario_{usuario.idusuario}_{int(datetime.now().timestamp())}"
            )
            
            print("Upload Cloudinary OK:", result['public_id'])
            
            # Salvar o public_id no banco
            usuario.imagem = result['public_id']
            usuario.save()
            
            # Gerar URL da imagem
            image_url = cloudinary.CloudinaryImage(usuario.imagem).build_url()
    ...
    return JsonResponse({
        'status': 'error', 
        'message': 'Método inválido ou arquivo não enviado'
    }, status=400)
...
def gerenciar_cursos_ajax(request):
    print("="*50)
    print("FUNÇÃO: gerenciar_cursos_ajax")
    print("Método:", request.method)
    print("Session:", request.session.get('usuario_email'))
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            print("Curso criado:", curso.idcurso)
            return JsonResponse({'status': 'success', 'id': curso.idcurso})
        
        elif request.method == 'PUT':
            print("PUT body:", request.body)
            data = json.loads(request.body)
            print("Dados edição:", data)
            ...
            print("DELETE body:", request.body)
            data = json.loads(request.body)
            print("Deletando curso ID:", data['id'])
...
def listar_projetos_ajax(request):
    ...
    try:
        ...
        # Pegar projetos que o usuário participa
        if usuario.tipo == "Professor":
            # Professor vê projetos das turmas que ele leciona
            turmas = Turma.objects.filter(professor=usuario)
            projetos = Projeto.objects.filter(turma__in=turmas)
        else:
            # Aluno vê projetos que ele participa
            projetos = usuario.projetos.all()
            ...
            projetos_list.append(
                {
                    ...
                    "data_criacao": projeto.data_de_criacao.strftime("%Y-%m-%d"),
                    "turma": f"{projeto.turma.curso.nome_curso} - {projeto.turma.ano} ({projeto.turma.turno})",
                }
            )

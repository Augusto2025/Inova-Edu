...
class UserProfileSystem:
    def __init__(self, parent_frame=None):
        """Inicializar sistema de perfil (pode funcionar independente ou integrado)"""
        ...
        if not parent_frame:
            # Modo independente
            self.app = ctk.CTk()
            self.app.title("Sistema de Perfil Acadêmico")
    ...
    def setup_ui(self, parent_frame):
        ...
        self.left_panel = ctk.CTkFrame(self.main_frame, width=350, corner_radius=15)
    ...
    def edit_bio(self):
        ...
        edit_window = ctk.CTkToplevel(self.parent if hasattr(self, 'parent') else self.app)
        edit_window.title("Editar Biografia")
        edit_window.geometry("500x400")
        edit_window.resizable(False, False)
        edit_window.transient(self.parent if hasattr(self, 'parent') else self.app)
    ...
    def add_certificate(self):
        ...
        parent_window = self.parent if hasattr(self, 'parent') else self.app
...
class CadastroTurmas:
    ...
    def criar_menu_lateral(self):
        ...
        # Opções do menu (adicionado botão Perfil)
        opcoes_menu = [
            "📚 Cadastro de Cursos",
            "👥 Cadastro de Turmas",
            "👤 Meu Perfil",  # Novo botão
            "",  # Espaço reservado
            "",  # Espaço reservado
            ""   # Espaço reservado
        ]
        ...
        for opcao in opcoes_menu:
            if opcao == "":
                # Espaço vazio
                espaco = ctk.CTkLabel(self.menu_frame, text="", height=10)
                espaco.pack(fill="x")
                continue
    ...
    def criar_tela_cadastro(self):
        ...
        self.titulo_conteudo.pack(side="left", padx=30, pady=20)
        
        # Área de conteúdo principal (inicialmente vazia para cadastro de turmas)
        self.area_conteudo = ctk.CTkFrame(
            self.conteudo_frame,
            fg_color=self.cor_branco
        )
        self.area_conteudo.pack(fill="both", expand=True, padx=20, pady=20)
        
        # Inicializar com tela de cadastro de turmas
        self.criar_conteudo_cadastro_turmas()
        
    def criar_conteudo_cadastro_turmas(self):
        """Criar conteúdo para cadastro de turmas"""
        # Limpar área de conteúdo
        for widget in self.area_conteudo.winfo_children():
            widget.destroy()
        
        # Título
        titulo_frame = ctk.CTkFrame(self.area_conteudo, fg_color="transparent")
        ...
        # Container para lista de turmas
        lista_frame = ctk.CTkFrame(self.area_conteudo, fg_color=self.cor_cinza_claro, corner_radius=10)
        ...
        colunas = ["Código", "Nome da Turma", "Curso", "Período", "Alunos", "Ações"]
        for col in colunas:
            ctk.CTkLabel(
                header_lista,
                text=col,
                font=ctk.CTkFont(size=14, weight="bold", family="Arial"),
                text_color=self.cor_azul,
                width=100 if col != "Nome da Turma" else 200
            ).pack(side="left", padx=5, fill="x", expand=(col == "Nome da Turma"))
        
        # Separador
        separador = ctk.CTkFrame(lista_frame, height=2, fg_color=self.cor_cinza)
        separador.pack(fill="x", padx=20)
        
        # Lista rolável
        scroll_frame = ctk.CTkScrollableFrame(lista_frame, fg_color="transparent")
        scroll_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        # Exemplo de turmas (dados fictícios)
        turmas_exemplo = [
            {"codigo": "TUR001", "nome": "Turma A", "curso": "Ciência da Computação", "periodo": "2023.1", "alunos": 30},
            {"codigo": "TUR002", "nome": "Turma B", "curso": "Engenharia de Software", "periodo": "2023.1", "alunos": 28},
            {"codigo": "TUR003", "nome": "Turma C", "curso": "Sistemas de Informação", "periodo": "2023.2", "alunos": 32},
        ]
        
        for turma in turmas_exemplo:
            linha = ctk.CTkFrame(scroll_frame, fg_color="white", height=45, corner_radius=5)
            ...
            # Ações
            acoes_frame = ctk.CTkFrame(linha, fg_color="transparent", width=150)
    ...
    def nova_turma(self):
        """Abrir formulário para nova turma"""
        messagebox.showinfo("Nova Turma", "Funcionalidade de nova turma seria implementada aqui.")
    
    def criar_conteudo_cadastro_cursos(self):
        """Criar conteúdo para cadastro de cursos"""
        # Limpar área de conteúdo
        for widget in self.area_conteudo.winfo_children():
            widget.destroy()
        ...
        info_frame = ctk.CTkFrame(self.area_conteudo, fg_color="#E8F4FD", corner_radius=10, height=150)
        ...
        ctk.CTkLabel(
            info_frame,
            text="Aqui você pode gerenciar todos os cursos oferecidos pela instituição.\nAdicione novos cursos, edite informações ou remova cursos desativados.",
            font=ctk.CTkFont(size=14, family="Arial"),
            text_color=self.cor_texto,
            justify="center"
        ).pack(pady=(0, 20))
    ...
    def selecionar_menu(self, opcao):
        ...
        # Atualizar conteúdo conforme opção selecionada
        if opcao == "📚 Cadastro de Cursos":
            self.titulo_conteudo.configure(text="📚 CADASTRO DE CURSOS")
            self.criar_conteudo_cadastro_cursos()
            
        elif opcao == "👥 Cadastro de Turmas":
            self.titulo_conteudo.configure(text="👥 CADASTRO DE TURMAS")
            self.criar_conteudo_cadastro_turmas()
            
        elif opcao == "👤 Meu Perfil":
            self.titulo_conteudo.configure(text="👤 MEU PERFIL")
            self.mostrar_perfil()
    ...
    def mostrar_perfil(self):
        ...
        # Criar frame para o perfil
        perfil_frame = ctk.CTkFrame(
            self.area_conteudo,
            fg_color="transparent"
        )
        perfil_frame.pack(fill="both", expand=True, padx=0, pady=0)
        
        # Inicializar sistema de perfil dentro do frame
        self.perfil_system = UserProfileSystem(perfil_frame)

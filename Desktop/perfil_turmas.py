...
class AppTurma(ctk.CTk):
    ...
    def criar_cards_usuarios(self):
        """Cria os cards para cada usuário com cores alternadas"""
        # Configurar grid - 4 colunas
        num_colunas = 4
        
        # Paleta de cores para os cards (CORES SÓLIDAS - sem transparência)
        cores_cards = [
            "#1e3a5a",  # Azul escuro
            "#5a1e3a",  # Vinho escuro
            "#1e5a3a",  # Verde escuro
            "#5a3a1e",  # Laranja escuro
            "#3a1e5a",  # Roxo escuro
            "#5a1e1e",  # Vermelho escuro
        ]
        ...
        for i, usuario in enumerate(self.usuarios):
            ...
            # Selecionar cor baseada no índice (alternada)
            cor_card = cores_cards[i % len(cores_cards)]
            
            # Criar frame para o card com cor personalizada
            card_frame = ctk.CTkFrame(
                ...
                corner_radius=15,
                border_width=2,
                border_color="#4a4a4a",  # COR SÓLIDA para a borda
                fg_color=cor_card
            )
            ...
            try:
                if usuario.foto_path and os.path.exists(usuario.foto_path):
                    ...
                    img = Image.new('RGB', (140, 140), color=(100, 100, 100))
